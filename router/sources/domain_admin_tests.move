...
module router::domain_admin_tests {
    ...
    fun test_domain_admin_set_subdomain_expiration_policy(
        ...
        // Set it to 1
        router::domain_admin_set_subdomain_expiration_policy(user, domain_name, subdomain_name, 1);
        assert!(router::get_subdomain_expiration_policy(domain_name, subdomain_name) == 1, 2);

        // Set it back to 0
        router::domain_admin_set_subdomain_expiration_policy(user, domain_name, subdomain_name, 0);
        assert!(router::get_subdomain_expiration_policy(domain_name, subdomain_name) == 0, 3);
    }
}
