module router::router {
    ...
    fun can_register_in_v2(
        ...
        let registerable_in_v1 = domains::name_is_expired_past_grace(subdomain_name,
            domain_name);
        if (registerable_in_v1) {
            v2_1_domains::is_name_registerable(domain_name, subdomain_name)
        } else {
            let (is_burned, _token_id) =
                domains::is_token_owner(router_signer_addr(), subdomain_name, domain_name);
            is_burned
        }
    }
    ...
    public entry fun register_domain(
        ...
        if (mode == MODE_V1) {
            ...
            domains::register_domain(user, domain_name, (
                    (registration_duration_secs / SECONDS_PER_YEAR) as u8
                ));
        } else if (mode == MODE_V1_AND_V2) {
            assert!(can_register_in_v2(domain_name, option::none()),
                error::unavailable(ENAME_NOT_AVAILABLE));
            v2_1_domains::register_domain(get_router_signer(), user, domain_name,
                registration_duration_secs,);
            // Clear the name in v1
            domains::force_clear_registration(get_router_signer(), option::none(),
                domain_name)
        } else {
            abort error::not_implemented(ENOT_IMPLEMENTED_IN_MODE)
        };
        ...
        set_target_addr(user, domain_name, option::none(), target_addr_with_default);
        ...
        set_primary_name_when_register(user, target_addr, to_addr, domain_name,
            option::none(),);
    }
    ...
    fun set_primary_name_when_register(
        ...
        if (option::is_some(&target_addr) && target_addr != option::some(owner_addr)) { return };
    ...
    public entry fun register_subdomain(
        ...
        let mode = get_mode();
        if (mode == MODE_V1) {
            domains::register_subdomain(user, subdomain_name, domain_name,
                expiration_time_sec);
        } else if (mode == MODE_V1_AND_V2) {
            assert!(can_register_in_v2(domain_name, option::some(subdomain_name)),
                error::unavailable(ENAME_NOT_AVAILABLE));
            v2_1_domains::register_subdomain(get_router_signer(),
                user,
                domain_name,
                subdomain_name,
                expiration_time_sec,);
            v2_1_domains::set_subdomain_expiration_policy(user, domain_name, subdomain_name,
                expiration_policy,);
            domains::force_clear_registration(get_router_signer(),
        ...
        set_target_addr(user, domain_name, option::some(subdomain_name),
        ...
        if (mode == MODE_V1_AND_V2) {
            v2_1_domains::set_subdomain_transferability_as_domain_owner(get_router_signer(), user,
                domain_name, subdomain_name, transferrable);
        };
    ...
    public entry fun migrate_name(
        ...
        if (mode == MODE_V1) {
            ...
            assert!(!exists_in_v2(domain_name, subdomain_name),
            ...
            let is_primary_name =
                if (option::is_some(&maybe_primary_name)) {
                    let (primary_subdomain_name, primary_domain_name) =
                        domains::get_name_record_key_v1_props(&option::extract(&mut maybe_primary_name));
            ...
            let tokendata_id =
                aptos_names::token_helper::build_tokendata_id(aptos_names::token_helper::get_token_signer_address(),
            ...
            if (option::is_some(&subdomain_name)) {
                assert!(exists_in_v2(domain_name, option::none()),
            ...
            aptos_token::token::direct_transfer(user, router_signer, token_id, 1,);
            ...
            let registration_duration_secs =
                if (new_expiration_time_sec > now) {
                    ...
                    1 };
            v2_1_domains::register_name_with_router(router_signer,
                user,
                domain_name,
                subdomain_name,
                registration_duration_secs,);
            ...
            if (is_primary_name) {
                ...
                v2_1_domains::set_target_address(user, domain_name, subdomain_name, *option::borrow(
                        &target_addr));
            };
    ...
    fun migrate_if_eligible(
        ...
        if (!exists_in_v2(domain_name, subdomain_name) && is_v1_name_owner(signer::address_of(user),
    ...
    fun has_primary_name(user: &signer,): bool acquires RouterConfig {
        ...
        if (mode == MODE_V1) {
            ...
            return(option::is_some(&domains::get_reverse_lookup(signer::address_of(user))) || option::is_some(
    ...
    public entry fun clear_primary_name(user: &signer) acquires RouterConfig {
        ...
        if (mode == MODE_V1) {
            ...
            // Clear primary name in v1 if exists so we do not have primary name in both v1 and v2
            let (v1_primary_subdomain_name, v1_primary_domain_name) = get_v1_primary_name(signer::address_of(
                    user));
            if (option::is_some(&v1_primary_domain_name)) {
                ...
                if (option::is_none(&v1_primary_subdomain_name)) {
                    migrate_name(user, *option::borrow(&v1_primary_domain_name),
    ...
    public entry fun set_target_addr(
        ...
        if (mode == MODE_V1) {
            ...
            migrate_if_eligible(user, domain_name, subdomain_name);
            v2_1_domains::set_target_address(user, domain_name, subdomain_name, target_addr,)
        } else {
            abort error::not_implemented(ENOT_IMPLEMENTED_IN_MODE)
        }
    }
    ...
    public entry fun domain_admin_transfer_subdomain(
        ...
        if (mode == MODE_V1) {
            ...
            v2_1_domains::transfer_subdomain_owner(domain_admin, domain_name, subdomain_name,
                to_addr, target_addr,)
        } else {
            abort error::not_implemented(ENOT_IMPLEMENTED_IN_MODE)
        }
    ...
    public entry fun domain_admin_set_subdomain_transferability(
        ...
        if (mode == MODE_V1) {
            ...
            v2_1_domains::set_subdomain_transferability_as_domain_owner(get_router_signer(),
                domain_admin,
                domain_name,
                subdomain_name,
                transferable)
    ...
    public entry fun domain_admin_set_subdomain_expiration_policy(
        ...
        if (mode == MODE_V1) {
            ...
            v2_1_domains::set_subdomain_expiration_policy(domain_admin, domain_name,
                subdomain_name, expiration_policy,)
        } else {
            abort error::not_implemented(ENOT_IMPLEMENTED_IN_MODE)
        }
    }
    ...
    public entry fun domain_admin_set_subdomain_expiration(
        ...
        if (mode == MODE_V1) {
            ...
            v2_1_domains::set_subdomain_expiration(domain_admin, domain_name, subdomain_name,
                expiration_time_sec,)
        } else {
            abort error::not_implemented(ENOT_IMPLEMENTED_IN_MODE)
        }
    ...
    inline fun is_v1_name_owner(
        ...
        let (is_owner, _token_id) = domains::is_token_owner(owner_addr, subdomain_name,
            domain_name);
    ...
    inline fun get_v1_primary_name(user_addr: address): (Option<String>, Option<String>) {
        ...
        if (option::is_none(&record)) { (option::none(), option::none()) }
        else {
    ...
    public fun get_primary_name(user_addr: address): (Option<String>, Option<String>) acquires RouterConfig {
        ...
        if (mode == MODE_V1) {
            ...
            if (!v2_1_domains::reverse_record_exists(user_addr)) {
                ...
                if (option::is_none(&token_addr)) { (option::none(), option::none()) }
                else {
                    let (subdomain_name, domain_name) =
                        v2_1_domains::get_name_props_from_token_addr(*option::borrow(&token_addr));
                    (subdomain_name, option::some(domain_name))
                }
            }
        } else {
    ...
    // == Transfer helper ==
    fun transfer_name(
        user: &signer, domain_name: String, subdomain_name: Option<String>, to_addr: address
    ) acquires RouterConfig {
        let mode = get_mode();
        if (mode == MODE_V1) {
            // Get the v1 token info
            ...
            let tokendata_id =
                aptos_names::token_helper::build_tokendata_id(aptos_names::token_helper::get_token_signer_address(),
                    subdomain_name, domain_name,);
            let token_id = aptos_names::token_helper::latest_token_id(&tokendata_id);
            aptos_token::token::transfer(user, token_id, to_addr, 1,);

            // TODO: Probably good idea to clear entries in v1
        } else if (mode == MODE_V1_AND_V2) {
            let token_addr = v2_1_domains::get_token_addr(domain_name, subdomain_name);
            object::transfer(user,
                object::address_to_object<v2_1_domains::NameRecord>(token_addr), to_addr,);
        } else {
            abort error::not_implemented(ENOT_IMPLEMENTED_IN_MODE)
        }
    }
