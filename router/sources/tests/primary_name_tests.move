...
module router::primary_name_tests {
    ...
    inline fun exists_in_v2(domain_name: String, subdomain_name: Option<String>): bool {
        object::is_object(aptos_names_v2::v2_1_domains::get_token_addr(domain_name, subdomain_name))
    }

    inline fun get_v2_primary_name(
        user_addr: address
    ): (Option<String>, Option<String>) {
        let token_addr = aptos_names_v2::v2_1_domains::get_reverse_lookup(user_addr);
        if (option::is_none(&token_addr)) {
            (option::none(), option::none())
        } else {
            let (subdomain_name, domain_name) = aptos_names_v2::v2_1_domains::get_name_props_from_token_addr(
    ...
    fun test_set_primary_name_should_trigger_auto_migration(
        ...
        router::set_primary_name(user, domain_name2, option::none());
        {
            ...
            assert!(aptos_names_v2::v2_1_domains::is_token_owner(user_addr, domain_name2, option::none()), 2);
            assert!(!aptos_names_v2::v2_1_domains::is_name_expired(domain_name2, option::none()), 3);
    ...
    fun test_clear_domain_primary_name_should_trigger_auto_migration(
        ...
        router::clear_primary_name(user);
        {
            ...
            assert!(!aptos_names_v2::v2_1_domains::is_name_expired(domain_name, option::none()), 2);
            // v1 primary name should be cleared
    ...
    fun test_clear_subdomain_primary_name_should_not_trigger_auto_migration(
        ...
        router::clear_primary_name(user);
        {
            ...
            assert!(is_owner_of_v1_name, 1);
            assert!(!aptos_names_v2::v2_1_domains::is_token_owner(user_addr, domain_name, subdomain_name_opt), 2);
            assert!(aptos_names_v2::v2_1_domains::is_name_expired(domain_name, subdomain_name_opt), 2);
            // v1 primary name should be cleared
            let (v1_primary_subdomain_name, v1_primary_domain_name) = get_v1_primary_name(user_addr);
            assert!(option::is_none(&v1_primary_domain_name), 2);
