...
module aptos_names::domain_e2e_tests {
    ...
    fun happy_path_e2e_test(
        ...
        // Register the domain
        test_helper::register_name(user,
            option::none(),
        ...
        test_helper::set_name_address(user, option::none(), test_helper::domain_name(),
            user_addr);
    ...
    fun names_are_registerable_after_expiry_e2e_test(
        ...
        let (_, expiration_time_sec, _) =
            domains::get_name_record_v1_props_for_name(option::none(),
                test_helper::domain_name());
        ...
        assert!(domains::name_is_registered(option::none(), test_helper::domain_name()),
            81);
        assert!(domains::name_is_registerable(option::none(), test_helper::domain_name()),
            82);

        // Lets try to register it again, now that it is expired
        test_helper::register_name(rando,
            option::none(),
            test_helper::domain_name(),
            test_helper::one_year_secs(),
            test_helper::fq_domain_name(),
        ...
        assert!(option::is_none(&domains::get_reverse_lookup(signer::address_of(user))),
            85);
    ...
    fun dont_allow_rando_to_set_domain_address_e2e_test(
        ...
        test_helper::set_name_address(rando, option::none(), test_helper::domain_name(),
            @aptos_names);
    }
    ...
    fun admin_can_force_seize_domain_name_e2e_test(
        ...
        let (is_owner, _token_id) =
            domains::is_owner_of_name(signer::address_of(user), option::none(),
                test_helper::domain_name());
        assert!(is_owner, 1);
        let (is_owner, _token_id) =
            domains::is_token_owner(signer::address_of(user), option::none(),
                test_helper::domain_name());
        assert!(is_owner, 1);
        ...
        domains::force_create_or_seize_name(myself, option::none(),
            test_helper::domain_name(), test_helper::two_hundred_year_secs());
        let (is_owner, _token_id) =
            domains::is_owner_of_name(signer::address_of(myself), option::none(),
                test_helper::domain_name());
        assert!(is_owner, 2);
        let (is_owner, _token_id) =
            domains::is_token_owner(signer::address_of(myself), option::none(),
                test_helper::domain_name());
        assert!(is_owner, 2);
        assert!(!domains::name_is_expired(option::none(), test_helper::domain_name()), 1);
        ...
        let (_, expiration_time_sec, _) =
            domains::get_name_record_v1_props_for_name(option::none(),
                test_helper::domain_name());
        assert!(time_helper::seconds_to_years(expiration_time_sec) == 200,
    ...
    fun admin_force_seize_domain_name_doesnt_clear_unrelated_primary_name_e2e_test(
        ...
        // Register the domain. This will be the user's reverse lookup
        {
            ...
            let (is_owner, _token_id) =
                domains::is_token_owner(signer::address_of(user), option::none(),
                    test_helper::domain_name());
            assert!(is_owner, 1);
            assert!(!domains::name_is_expired(option::none(), test_helper::domain_name()),
                1);
        };
        ...
        test_helper::register_name(user,
            ...
            vector::empty<u8>());
        let (is_owner, _token_id) =
            domains::is_owner_of_name(signer::address_of(user), option::none(), domain_name);
        assert!(is_owner, 1);
        let (is_owner, _token_id) =
            domains::is_token_owner(signer::address_of(user), option::none(), domain_name);
        assert!(is_owner, 1);
        assert!(!domains::name_is_expired(option::none(), test_helper::domain_name()), 1);

        // Take the domain name for much longer than users are allowed to register it for
        domains::force_create_or_seize_name(myself, option::none(), domain_name,
        ...
        assert!(is_owner, 1);
        let (is_owner, _token_id) =
            domains::is_token_owner(signer::address_of(myself), option::none(), domain_name);
        assert!(is_owner, 2);
    ...
    fun admin_can_force_set_expiration(
        ...
        domains::force_set_expiration(myself, option::none(), test_helper::domain_name(),
            test_helper::two_hundred_year_secs());
        {
            let (_, expiration_time_sec, _) =
                domains::get_name_record_v1_props_for_name(option::none(),
    ...
    fun only_admin_can_force_set_expiration(
        ...
        domains::force_set_expiration(user, option::none(), test_helper::domain_name(),
            test_helper::two_hundred_year_secs());
    }
    ...
    fun rando_cant_force_seize_domain_name_e2e_test(
        ...
        // Take the domain name for much longer than users are allowed to register it for
        domains::force_create_or_seize_name(rando, option::none(),
            test_helper::domain_name(), test_helper::two_hundred_year_secs());
    }
    ...
    fun set_primary_name_after_transfer_clears_old_primary_name_e2e_test(
        ...
        test_helper::register_name(user,
            ...
            vector::empty<u8>());
        let (is_owner, _) =
            domains::is_owner_of_name(user_addr, option::none(), test_helper::domain_name());
        assert!(is_owner, 1);
        let (is_owner, token_id) =
            domains::is_token_owner(user_addr, option::none(), test_helper::domain_name());
        assert!(is_owner, 1);
        assert!(!domains::name_is_expired(option::none(), test_helper::domain_name()), 1);
        ...
        assert!(option::is_some(&domains::get_reverse_lookup(user_addr)), 1);
        assert!(*option::borrow(&domains::name_resolved_address(option::none(),
                    test_helper::domain_name())) == user_addr,
            1);
        ...
        assert!(option::is_none(&domains::get_reverse_lookup(user_addr)), 1);
        assert!(*option::borrow(&domains::name_resolved_address(option::none(),
                    test_helper::domain_name())) == rando_addr,
            1);
    }
    ...
    fun clear_registration_property_version_test(
        ...
        while (i < 10) {
            ...
            domains::force_clear_registration(myself, option::none(),
                test_helper::domain_name());
            i = i + 1;
        };
    }
