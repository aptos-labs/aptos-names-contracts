...
module aptos_names::subdomain_e2e_tests {
    ...
    fun clear_registration_test(myself: &signer, user: signer, aptos: signer, rando: signer, foundation: signer) {
        ...
        let subdomain_opt = option::some(test_helper::subdomain_name());
        ...
        {
            let (is_owner, _token_id) = domains::is_owner_of_name(user_addr, subdomain_opt, test_helper::domain_name());
            assert!(is_owner, 1);
        };
        // Primary name is the domain
        {
        ...
        domains::clear_registration(user, subdomain_opt, test_helper::domain_name());

        assert!(!domains::name_is_registered(subdomain_opt, test_helper::domain_name()), 1);
        assert!(domains::name_is_registerable(subdomain_opt, test_helper::domain_name()), 1);
        {
            let (is_owner, _token_id) = domains::is_owner_of_name(user_addr, subdomain_opt, test_helper::domain_name());
            assert!(!is_owner, 1);
        };

        // Primary name continues to be the domain
        {
            let reverse_lookup = domains::get_reverse_lookup(user_addr);
            assert!(option::is_some(&reverse_lookup), 1);
            let (subdomain_name, domain_name) = domains::get_name_record_key_v1_props(option::borrow(&reverse_lookup));
    ...
    fun rando_cant_clear_registration_test(myself: &signer, user: signer, aptos: signer, rando: signer, foundation: signer) {
        ...
        domains::clear_registration(rando, subdomain_opt, test_helper::domain_name());
    }
}
