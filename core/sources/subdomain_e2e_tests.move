...
module aptos_names::subdomain_e2e_tests {
    ...
    fun happy_path_e2e_test(
        ...
        test_helper::set_name_address(user, option::some(test_helper::subdomain_name()),
            test_helper::domain_name(), user_addr);

        // Ensure these also work :-)
        test_helper::clear_name_address(user, option::some(test_helper::subdomain_name()),
            test_helper::domain_name());

        // And also can clear if is registered address, but not owner
        test_helper::set_name_address(user,
            option::some(test_helper::subdomain_name()),
            test_helper::domain_name(),
            signer::address_of(rando));
        test_helper::clear_name_address(rando, option::some(test_helper::subdomain_name()),
            test_helper::domain_name());
    }
    ...
    fun names_are_registerable_after_expiry_e2e_test(
        ...
        let (_, expiration_time_sec, _) =
            domains::get_name_record_v1_props_for_name(option::some(test_helper::subdomain_name()),
                test_helper::domain_name());
        ...
        assert!(domains::name_is_expired(option::some(test_helper::subdomain_name()),
                test_helper::domain_name()),
            90);
        assert!(domains::name_is_registered(option::some(test_helper::subdomain_name()),
                test_helper::domain_name()),
            91);
        assert!(!domains::name_is_registerable(option::some(test_helper::subdomain_name()),
                test_helper::domain_name()),
            92);
    ...
    fun dont_allow_rando_to_set_subdomain_address_e2e_test(
        ...
        test_helper::set_name_address(rando, option::some(test_helper::subdomain_name()),
            test_helper::domain_name(), @aptos_names);
    }
    ...
    fun admin_can_force_set_subdomain_address_e2e_test(
        ...
        domains::force_set_subdomain_address(myself, test_helper::subdomain_name(),
            test_helper::domain_name(), rando_addr);
        let (_property_version, _expiration_time_sec, target_address) =
            domains::get_name_record_v1_props_for_name(option::some(test_helper::subdomain_name()),
                test_helper::domain_name());
        test_utils::print_actual_expected(b"set_subdomain_address: ", target_address,
    ...
    fun admin_can_force_seize_subdomain_name_e2e_test(
        ...
        let (is_owner, _token_id) =
            domains::is_owner_of_name(signer::address_of(user),
                option::some(test_helper::subdomain_name()),
                test_helper::domain_name());
        assert!(is_owner, 1);
        let (is_owner, _token_id) =
            domains::is_token_owner(signer::address_of(user),
                option::some(test_helper::subdomain_name()),
                test_helper::domain_name());
        assert!(is_owner, 1);
        assert!(!domains::name_is_expired(option::some(test_helper::subdomain_name()),
        ...
        domains::force_create_or_seize_name(myself,
            option::some(test_helper::subdomain_name()),
            test_helper::domain_name(),
            test_helper::one_year_secs());
        let (is_owner, _token_id) =
            domains::is_owner_of_name(signer::address_of(myself),
                option::some(test_helper::subdomain_name()),
                test_helper::domain_name());
        assert!(is_owner, 2);
        let (is_owner, _token_id) =
            domains::is_token_owner(signer::address_of(myself),
                option::some(test_helper::subdomain_name()),
                test_helper::domain_name());
        assert!(is_owner, 2);
        assert!(!domains::name_is_expired(option::some(test_helper::subdomain_name()),
        ...
        let (_, expiration_time_sec, _) =
            domains::get_name_record_v1_props_for_name(option::some(test_helper::subdomain_name()),
                test_helper::domain_name());
        assert!(time_helper::seconds_to_years(expiration_time_sec) == 1,
    ...
    fun admin_can_force_set_expiration(
        ...
        domains::force_set_expiration(myself,
            option::some(test_helper::subdomain_name()),
            test_helper::domain_name(),
    ...
    fun rando_cant_force_seize_subdomain_name_e2e_test(
        ...
        domains::force_create_or_seize_name(rando,
            option::some(test_helper::subdomain_name()),
            test_helper::domain_name(),
            test_helper::two_hundred_year_secs());
    }
