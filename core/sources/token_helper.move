module aptos_names::token_helper {
    ...
    public fun get_token_signer_address(): address acquires CollectionCapabilityV1 {
        account::get_signer_capability_address(&borrow_global<CollectionCapabilityV1>(
                    @aptos_names).capability)
    }

    fun get_token_signer(): signer acquires CollectionCapabilityV1 {
        account::create_signer_with_capability(&borrow_global<CollectionCapabilityV1>(
                    @aptos_names).capability)
    }
    ...
    public(friend) fun initialize(framework: &signer) {
        ...
        let description = string::utf8(b".apt names from Aptos Labs");
        ...
        token::create_collection(&token_resource,
    ...
    public fun get_fully_qualified_domain_name(
        ...
        let subdomain_is_allowed =
            if (option::is_some(&subdomain_name)) {
                let (subdomain_is_allowed, _length) = utf8_utils::string_is_allowed(option::borrow(
                        &subdomain_name));
    ...
    public fun tokendata_exists(token_data_id: &TokenDataId): bool {
        let (creator, collection_name, token_name) = token::get_token_data_id_fields(
            token_data_id);
        token::check_tokendata_exists(creator, collection_name, token_name)
    }

    public fun build_tokendata_id(
        token_resource_address: address, subdomain_name: Option<String>, domain_name: String
    ): TokenDataId {
    ...
    public fun latest_token_id(token_data_id: &TokenDataId): TokenId {
        let (creator, _collection_name, _token_name) = token::get_token_data_id_fields(
            token_data_id);
    ...
    public(friend) fun ensure_token_data(
        ...
        let token_data_id =
            build_tokendata_id(signer::address_of(token_resource), subdomain_name,
                domain_name);
        if (tokendata_exists(&token_data_id)) { token_data_id }
        else {
    ...
    fun create_token_data(
        token_resource: &signer, subdomain_name: Option<String>, domain_name: String, type: String
    ): TokenDataId {
        // Set up the NFT
        let collection_name = config::collection_name_v1();
        assert!(token::check_collection_exists(signer::address_of(token_resource),
                collection_name),
        ...
        let token_mutate_config =
            token::create_token_mutability_config(&vector<bool>[false, true, true, false, true]);
        ...
        let property_values: vector<vector<u8>> = vector[property_map::borrow_value(&now), property_map::borrow_value(
                &type)];
        let property_types: vector<String> = vector[property_map::borrow_type(&now), property_map::borrow_type(
                &type)];

        token::create_tokendata(token_resource,
            collection_name,
            fq_domain_name,
            description,
            nft_maximum,
    ...
    public(friend) fun set_token_props(
        ...
        token::mutate_one_token(&token_resource,
            token_owner,
            token_id,
            property_keys,
            property_values,
