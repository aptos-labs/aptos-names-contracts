...
module aptos_names_v2::subdomain_e2e_tests {
    ...
    fun test_auto_renew_subdomain_e2e(
        ...
        // Register a subdomain!
        test_helper::register_name(router_signer, user, option::some(test_helper::subdomain_name()), test_helper::domain_name(), timestamp::now_seconds() + test_helper::one_year_secs(), test_helper::fq_subdomain_name(), 1, vector::empty<u8>());
        // The subdomain auto-renewal policy is true by default
        assert!(domains::get_subdomain_expiration_policy(test_helper::domain_name(), test_helper::subdomain_name()) == 0, 2);
    ...
    fun test_set_subdomain_renewal_policy(
        ...
        domains::set_subdomain_renewal_policy(user, test_helper::domain_name(), test_helper::subdomain_name(), 1);
        assert!(domains::get_subdomain_expiration_policy(test_helper::domain_name(), test_helper::subdomain_name()) == 1, 3);
    ...
    fun test_transfer_subdomain(
        ...
        // create the domain
        test_helper::register_name(router_signer, user, option::none(), test_helper::domain_name(), test_helper::one_year_secs(), test_helper::fq_domain_name(), 1, vector::empty<u8>());
        test_helper::register_name(router_signer, user, option::some(test_helper::subdomain_name()), test_helper::domain_name(), test_helper::one_year_secs(), test_helper::fq_domain_name(), 1, vector::empty<u8>());
    ...
    fun test_register_domain_less_than_a_year(
        ...
        let user = vector::borrow(&users, 0);
        // Register the domain
        test_helper::register_name(router_signer, user, option::none(), test_helper::domain_name(), 100, test_helper::fq_domain_name(), 1, vector::empty<u8>());
    }
    ...
    fun test_register_domain_duration_not_whole_years(
        ...
        let user = vector::borrow(&users, 0);
        // Register the domain
        test_helper::register_name(router_signer, user, option::none(), test_helper::domain_name(), test_helper::one_year_secs()+5, test_helper::fq_domain_name(), 1, vector::empty<u8>());
    }
    ...
    fun test_names_are_registerable_after_expiry_e2e(
        ...
        test_helper::register_name(router_signer, rando, option::none(), test_helper::domain_name(), test_helper::one_year_secs(), test_helper::fq_domain_name(), 2, vector::empty<u8>());
        ...
        test_helper::register_name(router_signer, rando, option::some(test_helper::subdomain_name()), test_helper::domain_name(), timestamp::now_seconds() + test_helper::one_year_secs(), test_helper::fq_subdomain_name(), 2, vector::empty<u8>());
        ...
        // Lets try to register it again, now that it is expired
        test_helper::register_name(router_signer, rando, option::none(), test_helper::domain_name(), test_helper::one_year_secs(), test_helper::fq_domain_name(), 3, vector::empty<u8>());
        // The subdomain should now be registerable: it's both expired AND the domain is registered
        assert!(domains::name_is_expired(option::some(test_helper::subdomain_name()), test_helper::domain_name()), 93);
        assert!(domains::name_is_registered(option::some(test_helper::subdomain_name()), test_helper::domain_name()), 94);
        assert!(domains::name_is_registerable(option::some(test_helper::subdomain_name()), test_helper::domain_name()), 95);
