module aptos_names_v2_1::v2_1_domains {
    ...
    fun init_module(account: &signer) {
        ...
        let constructor_ref =
            object::create_named_object(account, APP_OBJECT_SEED,);
        let extend_ref = object::generate_extend_ref(&constructor_ref);
        let app_signer = &object::generate_signer(&constructor_ref);
        collection::create_unlimited_collection(app_signer,
    ...
    fun create_token(
        ...
        let constructor_ref =
            token::create_named_token(&get_app_signer(),
    ...
    public fun register_domain(
        ...
        assert!(address_of(router_signer) == @router_signer,
        ...
        assert!(is_name_registerable(domain_name, subdomain_name),
            error::invalid_state(ENAME_NOT_AVAILABLE));

        let length = validate_name_string(domain_name);

        let price =
            v2_1_price_model::price_for_domain(length, registration_duration_secs);
        coin::transfer<AptosCoin>(sign, v2_1_config::fund_destination_address(), price);

        register_name_internal(sign, subdomain_name, domain_name, registration_duration_secs,
            price);
    }
    ...
    public fun register_subdomain(
        ...
        assert!(is_name_registerable(domain_name, option::some(subdomain_name)),
            error::invalid_state(ENAME_NOT_AVAILABLE));
        ...
        assert!(is_token_owner(signer_addr, domain_name, option::none()),
            error::permission_denied(ENOT_OWNER_OF_DOMAIN));
        assert!(!is_name_expired(domain_name, option::none()),
    ...
    fun register_name_internal(
        ...
        if (is_subdomain(subdomain_name)) {
            ...
            assert!(name_expiration_time_secs <= domain_record.expiration_time_sec,
        ...
        if (object::is_object(token_addr)) {
            ...
            object::transfer_with_ref(object::generate_linear_transfer_ref(&record.transfer_ref),
                account_addr);
        } else {
            create_token(account_addr, domain_name, subdomain_name,
                name_expiration_time_secs,);
        };

        event::emit_event<RegisterNameEvent>(&mut borrow_global_mut<RegisterNameEvents>(
                get_app_signer_addr()).register_name_events,
            RegisterNameEvent {
                domain_name,
                subdomain_name,
    ...
    public fun renew_domain(
        ...
        let price =
            v2_1_price_model::price_for_domain(length, renewal_duration_secs);
        // pay the price
        coin::transfer<AptosCoin>(sign, v2_1_config::fund_destination_address(), price);
            ...
            if (option::is_some(&record.target_address)) {
                let maybe_reverse_record =
                    if (exists<ReverseRecord>(*option::borrow(&record.target_address))) {
                        let reverse_record =
                            borrow_global<ReverseRecord>(*option::borrow(&record.target_address));
        ...
        // log the event
        event::emit_event<RenewNameEvent>(&mut borrow_global_mut<RenewNameEvents>(
                get_app_signer_addr()).renew_name_events,
            RenewNameEvent {
                domain_name,
    ...
    public fun set_subdomain_transferability_as_domain_owner(
        ...
        assert!(address_of(router_signer) == @router_signer,
            error::permission_denied(ENOT_ROUTER));
        validate_subdomain_registered_and_domain_owned_by_signer(sign, domain_name,
            subdomain_name);
    ...
    public fun set_subdomain_expiration(
        ...
        assert!(v2_1_config::is_enabled(), error::unavailable(ENOT_ENABLED));
        validate_subdomain_registered_and_domain_owned_by_signer(domain_admin, domain_name,
            subdomain_name);
        // check if the expiration time is valid
        let domain_record = get_record(domain_name, option::none());
        assert!(domain_record.expiration_time_sec >= expiration_time_sec,
        ...
        let record =
            borrow_global_mut<NameRecord>(get_token_addr_inline(domain_name,
                    subdomain_name_opt));
        ...
        assert!(subdomain_ext.subdomain_expiration_policy != SUBDOMAIN_POLICY_LOOKUP_DOMAIN_EXPIRATION,
    ...
    public fun set_target_address(
        sign: &signer, domain_name: String, subdomain_name: Option<String>, new_address: address
        ...
        let signer_addr = signer::address_of(sign);
        assert!(is_token_owner(signer_addr, domain_name, subdomain_name),
            error::permission_denied(ENOT_OWNER_OF_NAME));
        assert!(!is_name_expired(domain_name, subdomain_name),
    ...
    fun set_target_address_internal(
        ...
        assert!(is_name_registered(domain_name, subdomain_name),
        ...
        emit_set_target_address_event(subdomain_name,
            domain_name,
    ...
    public fun clear_target_address(
        ...
        assert!((is_owner && !is_expired)
            || is_target_address, error::permission_denied(ENOT_AUTHORIZED));
    ...
    public fun clear_reverse_lookup(account: &signer) acquires NameRecord, SubdomainExt, ReverseRecord, SetReverseLookupEvents {
        assert!(v2_1_config::is_enabled(), error::unavailable(ENOT_ENABLED));
    ...
    public fun get_reverse_lookup(account_addr: address): Option<address> acquires ReverseRecord, NameRecord {
    ...
    fun set_reverse_lookup_internal(account: &signer, token_addr: address,) acquires NameRecord, SubdomainExt, ReverseRecord, SetReverseLookupEvents {
        let account_addr = signer::address_of(account);
        let record_obj = object::address_to_object<NameRecord>(token_addr);
        assert!(object::owns(record_obj, account_addr),
        ...
        let record = borrow_global<NameRecord>(token_addr);
        emit_set_reverse_lookup_event(account_addr,
            prev_subdomain_name,
            prev_domain_name,
    ...
    fun clear_reverse_lookup_internal(account_addr: address) acquires NameRecord, SubdomainExt, ReverseRecord, SetReverseLookupEvents {
    ...
    public entry fun force_set_target_address(
        sign: &signer, domain_name: String, subdomain_name: Option<String>, new_owner: address
    ) acquires NameRecord, SubdomainExt, ReverseRecord, SetTargetAddressEvents, SetReverseLookupEvents {
    ...
    fun validate_name_string(name: String,): u64 {
        ...
        assert!(length <= v2_1_config::max_domain_length(),
            error::out_of_range(ENAME_TOO_LONG));
        assert!(length >= v2_1_config::min_domain_length(),
            error::out_of_range(ENAME_TOO_SHORT));

        return length
    }

    public fun is_domain_in_renewal_window(domain_name: String,): bool acquires NameRecord, SubdomainExt {
        // check if the domain is registered
        assert!(is_name_registered(domain_name, option::none()),
            error::not_found(ENAME_NOT_EXIST));
        // check if the domain is expired and past gract period already
        assert!(!is_name_expired_past_grace(domain_name, option::none()),
    ...
    fun get_app_signer(): signer acquires DomainObject {
        object::generate_signer_for_extending(&borrow_global<DomainObject>(
                get_app_signer_addr()).extend_ref)
    }

    inline fun get_token_addr_inline(
        domain_name: String, subdomain_name: Option<String>,
    ): address {
        token::create_token_address(&get_app_signer_addr(),
            &get_collection_name(is_subdomain(subdomain_name)),
            &v2_1_token_helper::get_fully_qualified_domain_name(subdomain_name, domain_name),)
    }
    ...
    fun get_record_obj(domain_name: String, subdomain_name: Option<String>,)
        : Object<NameRecord> {
    ...
    fun validate_registration_duration(registration_duration_secs: u64,) {
        assert!(registration_duration_secs != 0,
            error::invalid_argument(EDURATION_MUST_BE_WHOLE_YEARS));
        assert!(registration_duration_secs % SECONDS_PER_YEAR == 0,
            error::invalid_argument(EDURATION_MUST_BE_WHOLE_YEARS));

        assert!(registration_duration_secs <= v2_1_config::max_number_of_seconds_registered(),
    ...
    fun validate_subdomain_expiration_policy(
        ...
        assert!(subdomain_expiration_policy
            == SUBDOMAIN_POLICY_LOOKUP_DOMAIN_EXPIRATION || subdomain_expiration_policy
    ...
    fun validate_subdomain_registered_and_domain_owned_by_signer(
        ...
        assert!(is_name_registered(domain_name, option::some(subdomain_name)),
            error::not_found(ESUBDOMAIN_NOT_EXIST));
        // Ensure signer owns the domain we're registering a subdomain for
        assert!(is_token_owner(signer::address_of(sign), domain_name, option::none()),
            error::permission_denied(ENOT_OWNER_OF_DOMAIN));
    ...
    public fun is_name_expired_past_grace(
        ...
        else if (option::is_some(&subdomain_name) && is_subdomain_registered_before_domain(domain_name, *option::borrow(
                    &subdomain_name))) { true }
        else {
            let token_addr = get_token_addr_inline(domain_name, subdomain_name);
            let record = borrow_global<NameRecord>(token_addr);
    ...
    public fun is_name_registered(
        ...
        object::is_object(get_token_addr_inline(domain_name, subdomain_name)) && !object::is_owner(
    ...
    public fun is_token_owner(
        ...
        let record_obj =
            object::address_to_object<NameRecord>(get_token_addr_inline(domain_name,
                    subdomain_name));
    ...
    public fun get_name_owner_addr(
        ...
        if (!is_name_registered(domain_name, subdomain_name) || is_name_expired(domain_name,
                subdomain_name,))
        return option::none();
        let record_obj =
            object::address_to_object<NameRecord>(get_token_addr_inline(domain_name,
    ...
    public fun get_name_props_from_token_addr(token_addr: address): (Option<String>, String) acquires NameRecord, SubdomainExt {
        let record = borrow_global<NameRecord>(token_addr);
    ...
    fun emit_set_target_address_event(
        ...
        event::emit_event<SetTargetAddressEvent>(&mut borrow_global_mut<
                SetTargetAddressEvents>(get_app_signer_addr()).set_name_events,
            event,);
    }
    ...
    fun emit_set_reverse_lookup_event(
        ...
        event::emit_event<SetReverseLookupEvent>(&mut borrow_global_mut<
                SetReverseLookupEvents>(get_app_signer_addr()).set_reverse_lookup_events,
            event,);
    }
