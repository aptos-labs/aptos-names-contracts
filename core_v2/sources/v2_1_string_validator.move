module aptos_names_v2_1::v2_1_string_validator {
    ...
    fun bytes_are_allowed(bytes: &vector<u8>): (bool, u64) {
        ...
        let i = 0;
        let len = u64_characters.length();
        let allowed = true;

        while (i < len) {
            let c = u64_characters[i];
    ...
    friend fun string_is_allowed(string: &String): (bool, u64) {
        bytes_are_allowed(string.bytes())
    }
    ...
    friend fun utf8_to_vec_u64_fast(bytes: &vector<u8>): vector<u64> {
        ...
        while (i < len) {
            ...
            if (prefix < 8) {
                // 0xxx xxxx
                result.push_back(char1 as u64);
            } else if (prefix == 12 || prefix == 13) {
                // 110x xxxx  10xx xxxx
                let char1 = char1 as u64;
                let char2 = bytes[i + 1] as u64;
                result.push_back((char1 << 8) | char2);
                i += 1;
            } else if (prefix == 14) {
                // 1110 xxxx  10xx xxxx  10xx xxxx
                let char1 = char1 as u64;
                let char2 = bytes[i + 1] as u64;
                let char3 = bytes[i + 2] as u64;
                result.push_back((char1 << 16) | (char2 << 8) | char3);
                i += 2;
            } else if (prefix == 15) {
                ...
                let char2 = bytes[i + 1] as u64;
                let char3 = bytes[i + 2] as u64;
                let char4 = bytes[i + 3] as u64;
                result.push_back((char1 << 24) | (char2 << 16) | (char3 << 8) | char4);
                i += 3;
            } else {
    ...
    friend fun u128_to_string(value: u128): String {
        if (value == 0) {
            return string::utf8(b"0")
        };
        let buffer = vector<u8>[];
        while (value != 0) {
    ...
    fun test_latin_digits() {
        ...
        allowed_tests.reverse();
        let i = 0;
        let len = allowed_tests.length();
        while (i < len) {
            let example = allowed_tests.pop_back();
        ...
        not_allowed.reverse();
        let i = 0;
        let len = not_allowed.length();
        while (i < len) {
            let example = not_allowed.pop_back();
            let (was_allowed, length) = bytes_are_allowed(&example.text);
            assert!(!was_allowed, i);
            assert!(length == example.length, i);
            i += 1;
    ...
    fun test_utf8_to_vec_u64_fast() {
        ...
        assert!(res2 == vector[14727314], res2.pop_back());
        ...
        assert!(res4 == vector[4036989590], res4.pop_back());
    }
}
